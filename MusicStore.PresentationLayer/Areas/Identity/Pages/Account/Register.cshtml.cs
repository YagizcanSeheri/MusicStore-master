    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task OnGetAsync(string returnUrl = null)
        {
            ...
            Input = new InputModel() 
            {
                ...
                .Where(y=> y.Name != ProjectConstant.Role_User_Ind)
                .Select(z=> z.Name)
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (!await _roleManager.RoleExistsAsync(ProjectConstant.Role_Emp))
                    {
                        await _roleManager.CreateAsync(new IdentityRole(ProjectConstant.Role_Emp));
                    }
                    if (!await _roleManager.RoleExistsAsync(ProjectConstant.Role_User_Com))
                    {
                        await _roleManager.CreateAsync(new IdentityRole(ProjectConstant.Role_User_Com));
                    }
                    if (!await _roleManager.RoleExistsAsync(ProjectConstant.Role_User_Ind))
                    {
                        await _roleManager.CreateAsync(new IdentityRole(ProjectConstant.Role_User_Ind));
                    }
                    ...
                    else
                    {
                        if (user.CompanyId> 0)
                        {
                            await _userManager.AddToRoleAsync(user, ProjectConstant.Role_User_Com);
                        }
